...
namespace Managers
{
	...
	{
		[SerializeField]
		private string googleSheetId = "1RY9GYaMqRN-Yp4xQZqm0e-NdsvleIiSkHa5zWCK9Dn8";
		...
		{
			if (word.Length > 0 && sheetData.Length == 0)
				return false;
			...
			{
				int index = (min + max) / 2;
				int result = string.Compare(word, sheetData[index],
